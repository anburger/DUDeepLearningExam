...
'''
Modifications by Angela Burger: add possibility to run on Flowers-102 dataset
'''
...
def get_args():
    ...
    parser.add_argument('dataset', type=str, choices=['cifar10', 'cifar100', 'flowers-102'], help='Choose between Cifar10/100 and flowers-102.')
...
def test():
    ...
    if args.dataset == 'cifar10':
        ...
        #test_data = dset.Flowers102(root='./data', split='train', download=True, transform=train_transform) #use this as test data when using the 'test' as training set --> more images in test set
        nlabels = 102
    else:
        test_data = dset.CIFAR100(args.data_path, train=False, transform=test_transform, download=True)
        nlabels = 100
    ...
    print(state)
    print('Test error rate : {}'.format(incorrect/len(test_loader.dataset)))
